...
check_status() {
  ...
  then
    ...
    then
      ...
      then
        ...
        printf 'termination signal %d (%s)' "$((-${smallsh_status}))" "$(kill -l "$((-${smallsh_status}))")"
      fi
      printf '.\n'
    fi
  ...
  then
    ...
    then
      ...
      printf 'termination signal %d (%s)' "$((-${1:-0}))" "$(kill -l "$((-${1:-0}))")"
    fi
    printf ', instead got '
    if [ "${smallsh_status}" -ge 0 ]
    then
...
then
  ...
  do
    ...
    check_stdout
    check_child_count 1
    check_stderr < <(printf 'Child process %s done. Exit status %d.\n' "${smallsh_children[0]:-<PID>}" "${exit_val}")
  done
  printscore
fi
...
then
  ...
  do
    ...
    check_stdout
    check_child_count 1
    check_stderr < <(printf 'Child process %s done. Signaled %d.\n' "${smallsh_children[0]:-<PID>}" "${sig_val}")
  done
  printscore
fi
...
then
  ...
  check_stdout
  check_child_count 1
  check_stderr < <(printf 'Child process %s stopped. Continuing.\n' "${smallsh_children[0]:-<PID>}")
  printscore
fi
